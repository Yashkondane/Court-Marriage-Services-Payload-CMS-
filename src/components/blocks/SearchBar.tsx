...
export function SearchBar({ locations = [], services = [] }: SearchBarProps) {
  ...
  return (
    <form onSubmit={handleSearch} className="flex flex-wrap lg:flex-nowrap items-stretch bg-white border border-gray-200 rounded-lg overflow-hidden transition-all shadow-lg hover:shadow-xl">
      
      {/* City Select */}
      <div className="flex items-center gap-2.5 px-5 py-4 border-r border-gray-200 flex-1 min-w-[160px]">
        ...
        <div className="flex flex-col flex-1">
          <span className="text-[9px] font-black text-gray-400 uppercase tracking-[0.15em] leading-none mb-1">City</span>
          <select
            value={city}
            onChange={(e) => setCity(e.target.value)}
            className="bg-transparent text-[#111] text-sm font-semibold outline-none cursor-pointer appearance-none w-full"
          >
      ...
      {/* Legal Matter Select */}
      <div className="flex items-center gap-2.5 px-5 py-4 border-r border-gray-200 flex-1 min-w-[180px]">
        <svg className="w-4 h-4 text-[var(--color-secondary)] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col flex-1">
          <span className="text-[9px] font-black text-gray-400 uppercase tracking-[0.15em] leading-none mb-1">Legal Matter</span>
          <select
            value={matter}
            onChange={(e) => setMatter(e.target.value)}
            className="bg-transparent text-[#111] text-sm font-semibold outline-none cursor-pointer appearance-none w-full"
          >
            <option value="">e.g. Property Dispute</option>
            {services.map((svc) => (
      ...
      <button
        type="submit"
        className="btn-gold px-8 py-4 flex items-center gap-2 shrink-0 rounded-none text-xs"
      >
