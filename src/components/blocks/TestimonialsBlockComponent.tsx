  ...
  return (
    <div className="py-20 md:py-28 bg-[#fafafa] overflow-hidden">
      <div className="container-page px-4">
        {/* Header Section matching the image UI */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          {block.heading && (
            <div className="relative inline-block mb-6">
              ...
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-16 h-1 bg-[var(--color-secondary)]"></div>
            </div>
          )}
          {block.description && (
            <p className="text-lg text-gray-600 mt-6 leading-relaxed bg-clip-text">
              {block.description}
            </p>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {testimonials.map((t: any, i: number) => (
                <div key={i} className="bg-white p-8 rounded-sm shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-lg transition-shadow duration-300 relative group">
                  ...
                  {/* Top section with Stars and Large Quote icon */}
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex text-[var(--color-secondary)] gap-1">
                      {[...Array(5)].map((_, idx) => (
                        <FaStar key={idx} className={idx < Math.round(t.rating) ? 'text-[var(--color-secondary)]' : 'text-gray-200'} />
                      ))}
                    </div>
